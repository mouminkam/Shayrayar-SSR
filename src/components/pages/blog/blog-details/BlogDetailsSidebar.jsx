      ...
      <div className="single-sidebar-widget bg-white p-6 sm:p-8 lg:p-10 rounded-lg shadow-lg mb-8">
        ...
        <div className="search-widget">
          <form onSubmit={handleSearch} className="relative">
            ...
            <button
              type="submit"
              className="absolute right-0 top-0 w-16 sm:w-20 h-full bg-theme text-white rounded-r-lg hover:bg-title transition-colors duration-300 flex items-center justify-center"
            >
        ...
        <div className="news-widget-categories">
          <ul className="space-y-3">
            {categories.map((category, index) => (
              <li
                key={index}
                className={`bg-bg2 rounded-lg transition-all duration-300 ${
                  category.active ? "bg-theme" : "hover:bg-theme"
                }`}
              >
                <Link
                  href="/blog-details"
                  className={`flex items-center justify-between px-5 sm:px-6 py-5 ${
                    category.active ? "text-white" : "text-title hover:text-white"
                  }`}
                >
                  <span className="font-['Roboto',sans-serif] text-sm sm:text-base font-medium">
                    {category.name}
                  </span>
                  <span
                    className={`font-['Roboto',sans-serif] text-sm sm:text-base font-medium ${
                      category.active ? "text-white" : "text-title"
                    }`}
                  >
                    ({category.count})
                  </span>
      ...
      <div className="single-sidebar-widget bg-white p-6 sm:p-8 lg:p-10 rounded-lg shadow-lg mb-8">
        ...
        <div className="recent-post-area">
          {recentPosts.map((post, index) => (
            <div
              key={index}
              className={`recent-items flex items-center gap-4 sm:gap-5 ${
                index !== recentPosts.length - 1 ? "pb-5 mb-5 border-b border-gray-200" : ""
              }`}
            >
              ...
              <div className="recent-content">
                <ul className="flex items-center gap-2 mb-2">
                  <li className="flex items-center gap-1 text-theme font-['Epilogue',sans-serif] text-sm font-normal">
                    <Image
        ...
        <div className="tagcloud flex items-center flex-wrap gap-2 sm:gap-3">
          {tags.map((tag, index) => (
            <Link
              key={index}
              href="/blog-details"
              className={`inline-flex px-4 py-2 text-sm font-['Epilogue',sans-serif] font-normal capitalize rounded transition-all duration-300 ${
                index === 1
                  ? "bg-theme text-white"
                  : "bg-bg2 text-title hover:bg-theme hover:text-white"
              }`}
            >
              {tag}
            </Link>
