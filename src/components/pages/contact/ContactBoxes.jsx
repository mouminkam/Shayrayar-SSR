      ...
      <div className="container mx-auto mr-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 px-5 sm:px-10">
          {contactBoxes.map((box, index) => (
            <div
              key={index}
              className="bg-white border-2 border-theme  rounded-lg p-6 sm:p-8 lg:p-10 text-center h-full transition-all duration-300 hover:-translate-y-2 hover:shadow-lg group"
            >
              <div className="mb-6">
                <div className="inline-flex items-center justify-center p-4 sm:p-5 rounded-lg border border-theme
                 bg-bg2 group-hover:border-theme transition-colors duration-300">
                  <Image
                    src={box.icon}
                    alt={box.title}
                    width={48}
