...
export default function RegisterForm() {
  ...
  const { registerPhone, getGoogleAuthUrl, isLoading } = useAuthStore();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Save phone and password to sessionStorage for OTP verification and resend
        if (typeof window !== "undefined") {
          sessionStorage.setItem("registrationPhone", formData.phone.replace(/\s/g, ""));
        ...
        if (result.errors) {
          ...
          Object.keys(result.errors).forEach((key) => {
            const fieldName = key === "password_confirmation" ? "confirmPassword" : key;
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      toastError(error.message || "An error occurred while connecting to Google");
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="px-4 bg-bgimg text-text">Or</span>
        </div>
      </div>
